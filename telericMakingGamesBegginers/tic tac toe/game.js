...
xImg.style.right = window.innerWidth - canvas.width + 35 + "px";
...
const createGrid = () => {
    ...
    checkWin = () => {
        if(checkSecondaryDiagonal()){
            makeWinner(grid[0][dimensions - 1]);
        }
    ...
    makeWinner = (square) => {
        if(square.img.src === xImg.src){
            alert("Winner is X");
            xScoreHtml.innerText = "-" + ++xScore;
        }
        else {
            alert("Winner is C");
            circleScoreHtml.innerText = "-" + ++circleScore;
        }
    ...
    checkMainDiagonal = () => {
        for(let i = 0; i < dimensions - 1; i++){
            if(grid[i][i].img.src !== grid[i + 1][i + 1].img.src){
                return;
            }
        }
    ...
    checkSecondaryDiagonal = () => {
        for(let i = 0; i < dimensions - 1; i++){
            if(grid[i][dimensions - i - 1].img.src !== grid[i + 1][dimensions - (i + 1) - 1].img.src){
                return;
            }
        }
